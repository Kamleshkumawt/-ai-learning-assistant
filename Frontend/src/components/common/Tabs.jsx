            ...
            <nav className='flex gap-2'>
                {tabs.map((tab)=> (
                    ...
                    className={`relative pb-4 px-2 md:px-6 text-sm font-semibold transition-all duration-200 ${
                        activeTab === tab.name ? "text-emerald-500" : "text-slate-600 hover:text-slate-900"
                    }`}
                    >
                        <span className='relative z-10 '>{tab.label}</span>
                        {activeTab === tab.name && (
                            <div className='absolute bottom-0 left-0 right-0 h-0.5 bg-linear-to-r from-emerald-400 to-teal-500 rounded-full shadow-lg shadow-emerald-500/25'/>
                        )}
                        {activeTab == tab.name && (
