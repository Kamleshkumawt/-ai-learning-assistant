...
const ChatInterface = () => {
  ...
  const handleSendMessage = async (e) => {
    ...
    setHistory((prev) => [...prev, useMessage]);
    setMessage("");
    setLoading(true);
    try {
    ...
    return (
        <div key={index} className={`flex items-start gap-3 my-4 ${isUser ? 'justify-end' : ''}`}>
            ...
            <div className={`max-w-lg p-4 rounded-2xl shadow-sm ${
                isUser ? 'bg-linear-to-br from bg-emerald-500 to-teal-500 text-white rounded-br-md' 
    ...
    <div className="flex flex-col h-[70vh] bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 overflow-hidden">
      ...
      <div className="p-5 border-t border-slate-200/60 bg-white/80">
        <form onSubmit={handleSendMessage} className="flex items-center gap-3">
          ...
          className="shrink-0 w-12 h-12 bg-linear-to-br from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 disabled:cursor-not-allowed active:scale-95 flex items-center justify-center"
          >
